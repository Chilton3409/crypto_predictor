...
async def main():
    ...
    await client.collect_data(tickers)

    features_array = np.array(client.features_list)
    labels_array = np.array(client.labels_list)
    optimal_threshold = await client.calculate_optimal_threshold(features_array, labels_array)
    print(f"Optimal threshold: {optimal_threshold}")

    await client.generate_buy_signals(tickers=tickers, optimal_threshold=optimal_threshold)
    client.features_list.clear()
    client.labels_list.clear()
