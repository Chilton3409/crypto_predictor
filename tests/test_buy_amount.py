...
async def test_sell_trade_amount(ticker: str) -> str:
    ...
    asset = await client.get_asset_by_ticker(test, ticker='Cookie')
...
async def main():
    brekdown = await client.get_spot_positions()
    test = await client.sell_trade_amount(breakdown=brekdown, ticker='COOKIE')
