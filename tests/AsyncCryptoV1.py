...
class CryptoLinkClient():
    ...
    async def get_top_gainers(self):
        ...
        try:
            ...
            top_gainers_last_day = sorted_test[:20]  # Get the top 5 gainers
    ...
    async def sell_logic(self):
        try:
            ...
            for asset in spots:
                ...
                #create a new function for profit trade amount
                profit_trade_amount = await self.profit_trade_amount(trade_amount=trade_amount)
    ...
    async def bot_pnl(self, pnl:str):
        """
        take pnl after stop loss and take profit and add them to the 
        bots pnl stat
        """
        try:
            if pnl:
                pnl = Decimal(pnl)
                self.profit_and_loss += pnl
                return
        except Exception as e:
            logging.exception(msg=e)
            
    async def profit_trade_amount(self, trade_amount):
        """
    ...
    async def take_profits(self, pnl, profit_threshold, ticker, trade_amount):
        try:
            ...
            if pnl >= profit_threshold:
                print(f"taking profits on: {ticker} current pnl is: {pnl}")
                await self.create_sell_order(ticker=ticker, trade_amount=trade_amount)
                await self.bot_pnl(pnl=pnl)
            return
        except Exception as e:
            logging.exception(msg=e)
    ...
    async def stop_loss(self, pnl, stop_loss_threshold, ticker, trade_amount):
        try:
            ...
            if pnl <= stop_loss_threshold:
                print(f"stopping loss on {ticker} at: {pnl}. stop loss threshold is: {stop_loss_threshold}")
    ...
    async def cycle(self):
        ...
        try:
            ...
            await asyncio.gather(self.buy_logic(), self.sell_logic())
