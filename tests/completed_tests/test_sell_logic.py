...
async def test_sell_logic(breakdown, ticker: str):
    
    
    current_pnl = await test_unrealized_gains(breakdown=breakdown, ticker=ticker)
    
    #convert the current pnl into a decimal for eval
    pnl = Decimal(current_pnl)
    
    #check if pnl is above the take profit threshhold
    await test_take_profits(pnl=pnl, profit_threshold=10.0)
    await test_stop_loss(pnl, stop_loss_threshold=-5.00)
    return

async def test_take_profits(pnl, profit_threshold):
    try:
...
async def test_stop_loss(pnl, stop_loss_threshold):
    try:
...
async def main():
    ...
    tickers = await client.get_top_gainers()
    tickers = [ticker.replace('-USD', '') for ticker in tickers]
    for gainer in tickers:
        
        await client.sell_logic(ticker=gainer)
